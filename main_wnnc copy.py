...
parser.add_argument(
    "input",
    type=str,
    help="input point cloud file name, must have extension xyz/ply/obj/npy",
)
parser.add_argument(
    "--width_config",
    type=str,
    choices=["l0", "l1", "l2", "l3", "l4", "l5", "custom"],
    required=True,
...
parser.add_argument(
    ...
    type=float,
    default=0.01,
    help="only works if --width_config custom is specified",
)
parser.add_argument(
    "--wsmin",
    type=float,
...
print(points_unnormalized[0])

bbox_scale = 1.1
bbox_center = (points_unnormalized.min(0) + points_unnormalized.max(0)) / 2.0
...
widths = torch.ones_like(
    points_normalized[:, 0]
)  # we support per-point smoothing width, but do not use it in experiments
...
preset_widths = {
    "l0": [
        0.002,
        0.016,
    ],  # [0.002, 0.016]: noise level 0, used for uniform, noise free points in the paper
    "l1": [0.01, 0.04],  # [0.01, 0.04]: noise level 1, used for real scans in the paper
    "l2": [
        0.02,
        0.08,
    ],  # [0.02, 0.08]: noise level 2, used for sigma=0.25% in the paper
    ...
    "l5": [
        0.05,
        0.2,
    ],  # [0.05, 0.2]: noise level 5, used for sparse points and 3D sketches in the paper
...
print(
    f"[LOG] You are using width config {args.width_config} width wsmin = {wsmin}, wsmax = {wsmax}"
)
...
with torch.no_grad():
    ...
    for i in bar:
        width_scale = wsmin + ((args.iters - 1 - i) / ((args.iters - 1))) * (
            wsmax - wsmin
        )
...
with torch.no_grad():
    out_points_normals = np.concatenate(
        [points_unnormalized, out_normals.detach().cpu().numpy()], -1
    )
    np.savetxt(
        os.path.join(args.out_dir, os.path.basename(args.input)[:-4] + f".xyz"),
        out_points_normals,
    )
