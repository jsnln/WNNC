...
template<typename scalar_t>
scalar_t eval_A_mu(const scalar_t* diff, const scalar_t* mu, scalar_t smooth_width) {
    // diff = x - y
    // returns -(x - y)\cdot mu
    // user is responsible for making sure dim == 3
...
template<typename scalar_t>
void scatter_point_attrs_to_nodes_leaf_cpu_wrapper(
        const signedindex_t* ptr_node_parent_list,
        const scalar_t* ptr_points,
        const scalar_t* ptr_point_weights,
        const scalar_t* ptr_point_attrs,
...
template<typename scalar_t>
void scatter_point_attrs_to_nodes_nonleaf_cpu_wrapper(
        const signedindex_t* ptr_node_parent_list,
        const signedindex_t* ptr_node_children_list,
        const scalar_t* ptr_points,
        const scalar_t* ptr_point_weights,
...
template<typename scalar_t>
void find_next_to_scatter_cpu_wrapper(
        const signedindex_t* ptr_node_children_list,
        const bool* ptr_node_is_leaf_list,
        bool* ptr_scattered_mask,
        bool* ptr_next_to_scatter_mask,
...
std::vector<torch::Tensor> scatter_point_attrs_to_nodes_cpu(
    ...
    AT_DISPATCH_FLOATING_TYPES(points.type(), "scatter_point_attrs_to_nodes_leaf_cpu_wrapper", ([&] {
        scatter_point_attrs_to_nodes_leaf_cpu_wrapper<scalar_t>(
    ...
    // cpuMemcpy(ptr_scattered_mask_h, ptr_scattered_mask_d, sizeof(bool)*num_nodes, cpuMemcpyDeviceToHost);
    // cpuMemcpy(ptr_to_scatter_mask_h, ptr_to_scatter_mask_d, sizeof(bool)*num_nodes, cpuMemcpyDeviceToHost);
    // int num_scattered = 0;
    ...
    for (signedindex_t depth = tree_depth-1; depth >= 0; depth--) {
        AT_DISPATCH_FLOATING_TYPES(points.type(), "find_next_to_scatter_cpu_wrapper", ([&] {
            find_next_to_scatter_cpu_wrapper<scalar_t>(
        ...
        AT_DISPATCH_FLOATING_TYPES(points.type(), "scatter_point_attrs_to_nodes_nonleaf_cpu_wrapper", ([&] {
            scatter_point_attrs_to_nodes_nonleaf_cpu_wrapper<scalar_t>(
                node_parent_list.data<signedindex_t>(),
                node_children_list.data<signedindex_t>(),
                points.data<scalar_t>(),
                point_weights.data<scalar_t>(),
...
template<typename scalar_t>
void multiply_by_A_cpu_wrapper(
        const scalar_t* query_points,  // [N', 3]
        const scalar_t* query_width,   // [N',]
        const scalar_t* points,        // [N, 3]
        const scalar_t* point_attrs,   // [N, C]
...
torch::Tensor multiply_by_A_cpu(
    ...
    AT_DISPATCH_FLOATING_TYPES(points.type(), "multiply_by_A_cpu_wrapper", ([&] {
        multiply_by_A_cpu_wrapper<scalar_t>(
...
template<typename scalar_t>
void multiply_by_AT_cpu_wrapper(
        const scalar_t* query_points,  // [N', 3]
        const scalar_t* query_width,   // [N',]
        const scalar_t* points,        // [N, 3]
        const scalar_t* point_attrs,   // [N, C]
...
torch::Tensor multiply_by_AT_cpu(
    ...
    AT_DISPATCH_FLOATING_TYPES(points.type(), "multiply_by_AT_cpu_wrapper", ([&] {
        multiply_by_AT_cpu_wrapper<scalar_t>(
            query_points.data<scalar_t>(),  // [N', 3]
            query_width.data<scalar_t>(),   // [N',]
            points.data<scalar_t>(),        // [N, 3]
            point_attrs.data<scalar_t>(),   // [N, C]
...
template<typename scalar_t>
void multiply_by_G_cpu_wrapper(
        const scalar_t* query_points,  // [N', 3]
        const scalar_t* query_width,   // [N',]
        const scalar_t* points,        // [N, 3]
        const scalar_t* point_attrs,   // [N, C]
...
torch::Tensor multiply_by_G_cpu(
    ...
    AT_DISPATCH_FLOATING_TYPES(points.type(), "multiply_by_A_cpu_wrapper", ([&] {
        multiply_by_G_cpu_wrapper<scalar_t>(
            query_points.data<scalar_t>(),  // [N', 3]
            query_width.data<scalar_t>(),   // [N',]
            points.data<scalar_t>(),        // [N, 3]
            point_attrs.data<scalar_t>(),   // [N, C]
